	...
	for scanner.Scan() {
		...
		if strings.ContainsRune(line, '|') {
			...
			for i := 1; i < len(intSlice); i++ {
				...
				for _, prev := range intSlice[:i] {
					if contains(prev, precedesSlice) {
						validUpdate = false
						fmt.Println(fmt.Sprintf("Update %v is invalid because of %v in %v|%v ", intSlice, prev, current, precedesSlice))
						break
					}
				}
                if !validUpdate {
			...
			if !validUpdate {
                ...
                fmt.Println(fmt.Sprintf("Update %v is valid and we add %v to sum %v",line, midValue, sum))
				sum += midValue
			}
