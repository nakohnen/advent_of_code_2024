...
func find(allTowels []string, design string, memo map[string]int) int {
	if result, exists := memo[design]; exists {
        //fmt.Printf("Memo exists: %v for %s\n", result, design)
		return result
	} /**else {
        //fmt.Printf("There is no memo of %v\n", design)
    }**/
    ...
    for i:=1;i<lenDesign;i++ {
        ...
        part2 := find(allTowels, design[i:], memo)
        subResult := part1 * part2
        //fmt.Printf("%v We need to check %v and %v => %d\n", design, design[:i], design[i:], subResult)
        //fmt.Printf("Results: %v * %v = %v\n", part1, part2, subResult)
        result += subResult
    }
	memo[design] = result
	return result
