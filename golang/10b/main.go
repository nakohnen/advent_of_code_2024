...
func walkMap(start point, topoMap [][]int, adjTopoMap map[point][]point, endValue int) []string {
	result := []string{}

	candidates := []trail{}
	for _, other := range adjTopoMap[start] {
		t := trail{p: other, history: getDirection(start, other)}
	...
	for len(candidates) > 0 {
		...
		if topoMap[nextPos.p.y][nextPos.p.x] == endValue {
			result, _ = appendIfNew(result, nextPos.history)
			continue
		}

		for _, newCandidate := range adjTopoMap[nextPos.p] {
			newTrail := trail{p: newCandidate, history: nextPos.history + getDirection(nextPos.p, newCandidate)}
			candidates = append(candidates, newTrail)
		}
