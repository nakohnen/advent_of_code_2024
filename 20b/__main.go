...
func search(tilemapOrig map[point]bool, maxWidth, maxHeight int, start, end point, skip skip, getPath bool) (int, []point) {
    ...
    return gScore[end], fullPath.GetElements()
...
func main() {
    ...
    finalScore, fullPath  := search(tilemap, maxWidth, maxHeight, start, end, skip{start, start}, true)

    {
        worker := func(id int, jobs <-chan point, wg *sync.WaitGroup, mu *sync.Mutex, scores map[point]int) {
            defer wg.Done()

            for job := range jobs {
                result, _ := search(tilemap, maxWidth, maxHeight, job, end, skip{job, job}, false)
                mu.Lock()
                scores[job] = result
                //fmt.Printf("Score: %v of %v\n", result, job)
                mu.Unlock()
            }
        }

        // Inputs and setup
        jobs := make(chan point, path.Size())
        ...
        for i := 1; i <= numWorkers; i++ {
            wg.Add(1)
            go worker(i, jobs, &wg, &mu, scores)
        }

        // Send jobs
        for _, input := range path.GetElements()  {
            jobs <- input
        }
        close(jobs)
    ...
    fmt.Printf("Length of best path: %d\n", len(fullPath))
    ...
    for i:=0;i<len(pathSlice);i++ {
        for j:=i+1;j<len(pathSlice);j++{
            ...
            if ManhattanDistance(p1, p2) <= 70 && abs(scores[p1] - scores[p2]) >= 100 {
    ...
    worker := func(id int, jobs <-chan skip, results chan<- int, wg *sync.WaitGroup) {
        ...
        for job := range jobs {
            result, _ := search(tilemap, maxWidth, maxHeight, start, end, job, false)
