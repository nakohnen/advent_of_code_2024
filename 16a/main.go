...
func search(adjMap map[path]map[path]int, start path, end point) int {
	openSet := []path{start}
	...
	return MinSlice([]int{gScore[path{end, East}], gScore[path{end, North}], gScore[path{end, West}], gScore[path{end, South}]})
}
